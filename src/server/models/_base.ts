...
const statics = {
	assertFindOne: async function (filter?: FilterQuery<IBase>, options?: Options<IBase>, projection?: Projection) {
		const doc = await this.findOne(filter, projection, options);
		if (!doc) throw new NotFoundError(`${this.modelName} not found`);
		return doc;
	},
	assertFind: async function (filter?: FilterQuery<IBase>, options?: Options<IBase>, projection?: Projection) {
	...
	getCount: async function (filter: FilterQuery<IBase>, options?: Options<IBase>) {
		return this.find(filter, options).count();
	},
	assertExists: async function (filter: FilterQuery<IBase>, options?: Options<IBase>) {
